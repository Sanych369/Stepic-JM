        ...
        if (mail instanceof MailMessage) {
            ...
            if (mailMessage.getFrom().equals(AUSTIN_POWERS) 
              || mailMessage.getTo().equals(AUSTIN_POWERS)) {
                this.logger.log(Level.WARNING, "Detected target mail correspondence: from " + mailMessage.getFrom() +
                        " to " + mailMessage.getTo() + " \"" + mailMessage.getMessage() + "\"");
            } else {
                this.logger.log(Level.INFO, "Usual correspondence: from " + mailMessage.getFrom()
...
public static class Thief implements MailService {
    ...
    public Sendable processMail(Sendable mail) {
        if (mail instanceof MailPackage) {
            ...
            if (mailPackage.getContent().getPrice() >= this.minCount && mailPackage.getContent().getPrice() != 0) {
                this.StolenCount += mailPackage.getContent().getPrice();
